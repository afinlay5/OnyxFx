...
//RoboVM has no support for Java8's java.util.function.BiConsumer.andThen();
// import me.adriandavid.please.oracle.dont.sue.BiConsumer;
//RoboVM has no support for Java8's java.util.Map.forEach();
// import me.adriandavid.please.oracle.dont.sue.Map8;
...
public class OnyxFx extends Application {
	...
	static class NBAPlayer {
		...
		NBAPlayer setPlayer(String firstName, String lastName, int season) { 
			//Java 8 Map.replace() API not supported
			this.playerInfo.put("firstName", firstName);
			this.playerInfo.put("lastName", lastName);
			this.playerInfo.put("season", String.valueOf(season));
			return player;
			...
			try {
				if (T.test(apiURL)) {
					if (  OnyxFx.firstn_ans != null &&
						...
						if (json.contains("-1")) {
							//Java 8 Map.replace() API not supported
							this.playerInfo.put("firstName", "Invalid Entry");
							this.playerInfo.put("lastName", "Invalid Entry");
							this.playerInfo.put("season", "Invalid Entry");
    ...
    public void start(Stage main) {
        ...
        firstn_ans.setOnAction((e) -> {
			...
			List<String> pg = Arrays.asList(statLine.keySet().toArray(new String[OnyxFx.PG_CONST]));
			for (int i=0; i<OnyxFx.PG_CONST; i++) {
				rsps.get(i).setText( pg.get(i) );
			}
          } 
        );
